...
import constants
import AI.constants as root_constants
...
def convertTrend(value):
    if np. isnan(value):
        ...
        trend_value = constants.TREND_WEAK_LABEL
    elif value < constants.TREND_NORMAL_MAX:
        trend_value = constants.TREND_NORMAL_LABEL
    elif value < constants.TREND_STRONG_MAX:
        trend_value = constants.TREND_STRONG_LABEL
    else:
        trend_value = constants.TREND_VERY_STRONG_LABEL
...
def convertUpDown(open, close):
    if close - open == 0:
        today_updown = constants.TODAY_NO_MOVE_LABEL
    elif close - open > 0:
        today_updown = constants.TODAY_IS_UP_LABEL
    else:
        today_updown = constants.TODAY_IS_DOWN_LABEL
...
if __name__ == "__main__":
    vn30f1m_file = '{0}/VN30ps/VN30F1M.csv'.format(str(root_constants.AI_DIR))
    ...
    save_data.to_csv(str(current_folder) + '/VN30F1M_for_decision_tree_label.csv')
    exit()
